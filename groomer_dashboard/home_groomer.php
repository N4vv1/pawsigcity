        ...
        <tbody>
          <?php while ($row = pg_fetch_assoc($result)): ?>
            <tr class="appointment-row" 
                ...
                data-customer="<?= strtolower(htmlspecialchars($row['username'])) ?>">
...
function completeAppointment(appointmentId) {
  ...
  .then(data => {
    if (data.success) {
      ...
      // Update filtered rows array
      const rowIndex = filteredRows.findIndex(r => r.id === 'row-' + appointmentId);
      if (rowIndex > -1) {
      ...
      const visibleRows = Array.from(tbody.querySelectorAll('.appointment-row'));
...
function filterAppointments() {
  ...
  rows.forEach(row => {
    ...
    if (matchesSearch && matchesDate && matchesPackage) {
