...
// Function to send email via SendGrid API (Alternative to SMTP)
...
// Function to send email via Mailgun API (Alternative)
function sendOtpEmailMailgun($email, $otp, $subject, $message) {
    $mailgun_domain = getenv('MAILGUN_DOMAIN') ?: 'YOUR_MAILGUN_DOMAIN';
...
// Fallback: Try SMTP with extended timeout
function sendOtpEmailSMTP($email, $otp, $subject, $message) {
    ...
    try {
        // Server settings - with extended timeouts for Render
        $mail->isSMTP();
        ...
        // Extended timeouts for cloud hosting
        $mail->Timeout = 60;
        $mail->SMTPDebug = 0;
        $mail->SMTPOptions = [
            'ssl' => [
...
function sendOtpEmail($email, $otp, $subject, $message) {
    // Try API-based methods first (more reliable on cloud hosting)
    
    // 1. Try SendGrid if API key is set
    ...
    // 2. Try Mailgun if API key is set
